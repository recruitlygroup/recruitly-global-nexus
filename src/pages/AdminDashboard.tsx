...
if (!isAdmin) { navigate("/not-found", { replace: true }); return; }
    ...
    return (
      <div className="min-h-screen bg-[#0a192f] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          <div className="w-12 h-12 rounded-xl flex items-center justify-center" style={{background:"rgba(251,191,36,0.2)"}}>
  ...
  // Plain div stat cards — NOT using shadcn <Card> because it injects bg-card (white) via CSS vars
  const statCards = [
    ...
    { label:"Applications",     value:stats.jobApplications, icon:ClipboardList, color:"text-lime-400",   bg:"bg-lime-500/10 border-lime-500/20"     },
    { label:"Candidates",       value:stats.candidates,      icon:UserCheck,     color:"text-cyan-400",   bg:"bg-cyan-500/10 border-cyan-500/20"     },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#0a192f]">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {statsError && (
              <button type="button" onClick={fetchStats}
                className="hidden sm:flex items-center gap-1.5 text-[11px] text-amber-400 hover:text-amber-300 border border-amber-500/20 rounded-lg px-2.5 py-1.5 transition-colors"
      ...
      <main className="max-w-screen-2xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        {/* ── STAT CARDS — explicit dark bg via border + bg utility, avoids CSS var bleed ── */}
        <div>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-white/70 text-xs font-semibold uppercase tracking-widest">Overview</h2>
            <button type="button" onClick={fetchStats} disabled={statsLoading}
