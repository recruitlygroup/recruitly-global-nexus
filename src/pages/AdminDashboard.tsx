...
// CHANGE: Added Consultations tab using AdminConsultationsTab component.
// All other tabs unchanged. Framer-motion removed from stat cards.
...
import AdminConsultationsTab   from "@/components/admin/AdminConsultationsTab"; // NEW
...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      if (error || !isAdmin) { navigate("/not-found"); return; }
  ...
  const tabs = [
    ...
    { value: "consultations", label: "Consultations",  icon: PhoneCall     },  // NEW
  ...
  return (
    <div className="min-h-screen bg-[#0a192f]">
      ...
      <main className="container mx-auto px-4 py-8">
        {/* Stats — no framer-motion (performance fix) */}
          ...
          {statCards.map(s => (
            <Card key={s.label} className={`${s.bg} border`}>
              <CardContent className="pt-4 pb-4 px-4">
                ...
                <p className={`${s.color} text-xs leading-tight mt-0.5`}>{s.label}</p>
              </CardContent>
            </Card>
          ))}
        </div>
        ...
        <Tabs defaultValue="candidates" className="space-y-4">
          ...
          <TabsContent value="consultations"><AdminConsultationsTab /></TabsContent>
          ...
          <TabsContent value="wisescore">
            <AdminDataTab
              title="WiseScore Leads" action="get_wisescore_leads" dataKey="leads"
          ...
          <TabsContent value="visa">
            <AdminDataTab
              title="Visa Predictions" action="get_visa_predictions" dataKey="predictions"
